...
class dgia():
    ...
    def __init__(self, model,model2, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        """Initialize with algorithm setup."""
        self.config = _validate_config(config)
        self.model = model
        self.model2=model2
    ...
    def reconstruct(self, input_data,input_data2, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        """Reconstruct image from gradient."""
        start_time = time.time()
        if eval:
            self.model.eval()
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels = self._run_trial(x[trial], input_data, input_data2, labels, dryrun=dryrun)
                # Finalize
                scores[trial] = self._score_trial(x_trial, input_data, input_data2, labels)
                x[trial] = x_trial
                if tol is not None and scores[trial] <= tol:
                    break
                if dryrun:
        ...
        # Choose optimal result:
        if self.config['scoring_choice'] in ['pixelmean', 'pixelmedian']:
            x_optimal, stats = self._average_trials(x, labels, input_data, input_data2, stats)
        else:
            print('Choosing optimal result ...')
            scores = scores[torch.isfinite(scores)]  # guard against NaN/-Inf scores?
            optimal_index = torch.argmin(scores)
    ...
    def _run_trial(self, x_trial, input_data, input_data2, labels, dryrun=False):
        x_trial.requires_grad = True
        if self.reconstruct_label:
            output_test = self.model(x_trial)
            labels = torch.randn(output_test.shape[1]).to(**self.setup).requires_grad_(True)
        ...
        try:
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial, input_data, input_data2, labels)
                rec_loss = optimizer.step(closure)
                if self.config['lr_decay']:
                    scheduler.step()
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, input_gradient2, label):

        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            loss = self.loss_fn(self.model(x_trial), label)
            loss2=self.loss_fn(self.model2(x_trial), label)
            gradient = torch.autograd.grad(loss, self.model.parameters(), create_graph=True)
            gradient2 = torch.autograd.grad(loss2, self.model2.parameters(), create_graph=False)
            rec_loss = reconstruction_costs_dgia([gradient],[gradient2], input_gradient,input_gradient2,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])
    ...
    def _score_trial(self, x_trial, input_gradient,input_gradient2, label):
        if self.config['scoring_choice'] == 'loss':
            self.model.zero_grad()
            x_trial.grad = None
            loss = self.loss_fn(self.model(x_trial), label)
            ...
            gradient = torch.autograd.grad(loss, self.model.parameters(), create_graph=False)
            gradient2 = torch.autograd.grad(loss2, self.model2.parameters(), create_graph=False)
            return reconstruction_costs_dgia([gradient],[gradient2], input_gradient,input_gradient2,
                                        cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                        weights=self.config['weights'])
        elif self.config['scoring_choice'] == 'tv':
            return TV(x_trial)
    ...
    def _average_trials(self, x, labels, input_data, input_data2, stats):
        print(f'Computing a combined result via {self.config["scoring_choice"]} ...')
        if self.config['scoring_choice'] == 'pixelmedian':
            x_optimal, _ = x.median(dim=0, keepdims=False)
        elif self.config['scoring_choice'] == 'pixelmean':
        ...
        if self.reconstruct_label:
            labels = self.model(x_optimal).softmax(dim=1)
        loss = self.loss_fn(self.model(x_optimal), labels)
        loss2=self.loss_fn(self.model2(x_optimal), labels)
        gradient = torch.autograd.grad(loss, self.model.parameters(), create_graph=False)
        gradient2 = torch.autograd.grad(loss2, self.model2.parameters(), create_graph=False)
        stats['opt'] = reconstruction_costs_dgia([gradient],[gradient2], input_data, input_data2,
                                            cost_fn=self.config['cost_fn'],
                                            indices=self.config['indices'],
                                            weights=self.config['weights'])
        print(f'Optimal result score: {stats["opt"]:2.4f}')
...
def reconstruction_costs_dgia(gradients,gradients2, input_gradient, input_gradient2, cost_fn='l2', indices='def', weights='equal'):
    """Input gradient is given data."""
    if isinstance(indices, list):
        pass
    elif indices == 'def':
    ...
    total_costs = 0
    for trial_gradient,trial_gradient2 in zip(gradients,gradients2):
        pnorm = [0, 0]
        costs = 0
        if indices == 'topk-2':
            _, indices = torch.topk(torch.stack([p.norm().detach() for p in trial_gradient], dim=0), 4)
        for i in indices:
            if cost_fn == 'l2':
                costs += ((trial_gradient[i]-trial_gradient2[i] - input_gradient[i]+ input_gradient2[i]).pow(2)).sum() * weights[i]
            elif cost_fn == 'l1':
                costs += ((trial_gradient[i]-trial_gradient2[i]  - input_gradient[i]+ input_gradient2[i]).abs()).sum() * weights[i]
            elif cost_fn == 'max':
                costs += ((trial_gradient[i]-trial_gradient2[i]  - input_gradient[i]+ input_gradient2[i]).abs()).max() * weights[i]
            elif cost_fn == 'sim':
                costs -= ((trial_gradient[i]+trial_gradient2[i] ) * (input_gradient[i]- input_gradient2[i])).sum() * weights[i]
                pnorm[0] += (trial_gradient[i]-trial_gradient2[i] ).pow(2).sum() * weights[i]
                pnorm[1] += (input_gradient[i]-input_gradient2[i]).pow(2).sum() * weights[i]
            elif cost_fn == 'simlocal':
                costs += 1 - torch.nn.functional.cosine_similarity((trial_gradient[i]-trial_gradient2[i] ).flatten(),
                                                                   (input_gradient[i]-input_gradient2[i]).flatten(),
                                                                   0, 1e-10) * weights[i]
        if cost_fn == 'sim':
            costs = 1 + costs / pnorm[0].sqrt() / pnorm[1].sqrt()
