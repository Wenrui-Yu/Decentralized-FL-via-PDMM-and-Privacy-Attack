...
if __name__=='__main__':
    ...
    proportion=50000//args.num_of_clients//args.batchsize
    ...
    for j in range(args.num_of_clients):
        nodes.append(node(AA[j, :], torch.utils.data.DataLoader(TensorDataset(split_datasets[j][0],split_datasets[j][1]), batch_size=args.batchsize, shuffle=True),Net(),device))
    #initialize z
    dual=[[] for i in range(args.num_of_clients)]
    worker_state_dict = Net().state_dict()
    ...
    for param_tensor in weight_keys:
        for n in range(args.num_of_clients):
            dual[n].append(torch.randn(worker_state_dict[param_tensor].size(), device=device)*args.z_std)
    for j in range(args.num_of_clients):
        nodes[j].init_dual(dual)
    ...
    for k in range(args.num_comm):
        ...
        y_tmp=[]
        for i in range(args.num_of_clients):
            loss,yij=nodes[i].active_update(device,args.epo)
            y_tmp.append(yij)
        for i in range(args.num_of_clients):
            for n,j in enumerate(np.flatnonzero(nodes[i].neighbor)):
                nodes[j].second_update(device,y_tmp[i],i,j)
        ...
        if k % args.val_freq == 0:
            ...
            scio.savemat('loss_ni{}_N{}_z{}_i{}_e{}.mat'.format(args.batchsize,args.num_of_clients,args.z_std,args.IID,args.epo), {'loss':sav,'prediction':cor})

        if args.model_t<=k<args.model_t+3:
            mod=[]
            for i in range(args.num_of_clients):
                mod.append(nodes[i].model.state_dict())
            with open('model_ni{}_N{}_t{}_z{}_e{}.pkl'.format(args.batchsize,args.num_of_clients,k,args.z_std,args.epo), 'wb') as f:
                pickle.dump(mod, f)
